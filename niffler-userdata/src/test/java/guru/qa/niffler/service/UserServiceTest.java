...
class UserServiceTest {
    ...
    @Test
    void getCurrentUserShouldReturnUserWhenExists(@Mock UserRepository userRepository) {
        when(userRepository.findByUsername(eq(mainTestUserName)))
                .thenReturn(Optional.of(mainTestUser));

        userService = new UserService(userRepository);

        UserJson result = userService.getCurrentUser(mainTestUserName);
    ...
    @Test
    void getCurrentUserShouldReturnDefaultWhenNotExists(@Mock UserRepository userRepository) {
        when(userRepository.findByUsername(eq(notExistingUser)))
                .thenReturn(Optional.empty());
    ...
    @Test
    void allUsersShouldCallCorrectRepositoryMethodBasedWithQuery(@Mock UserRepository userRepository) {
        ...
        List<UserJsonBulk> resultWithQuery = userService.allUsers(mainTestUserName, searchQuery);

        assertEquals(1, resultWithQuery.size());
        verify(userRepository, times(1)).findByUsernameNot(mainTestUserName, searchQuery);
        verify(userRepository, never()).findByUsernameNot(mainTestUserName);
    }

    @Test
    void allUsersShouldCallCorrectRepositoryMethodBasedWithoutQuery(@Mock UserRepository userRepository) {
        List<UserWithStatus> mockUsers = List.of(
                new UserWithStatus(
                        secondTestUser.getId(),
                        secondTestUser.getUsername(),
        ...
        List<UserJsonBulk> resultWithQuery = userService.allUsers(mainTestUserName, null);

        assertEquals(1, resultWithQuery.size());
        verify(userRepository, never()).findByUsernameNot(mainTestUserName, searchQuery);
        verify(userRepository, times(1)).findByUsernameNot(mainTestUserName);
    }

    @Test
    void friendsShouldCallCorrectRepositoryMethodBasedWithQuery(@Mock UserRepository userRepository) {
        UserWithStatus friend =
                new UserWithStatus(
                        secondTestUser.getId(),
                        secondTestUser.getUsername(),
                        secondTestUser.getCurrency(),
        ...
        when(userRepository.findByUsername(mainTestUserName))
                .thenReturn(Optional.of(mainTestUser));
        when(userRepository.findFriends(mainTestUser, secondTestUserName))
                .thenReturn(List.of(friend));

        userService = new UserService(userRepository);

        List<UserJsonBulk> result = userService.friends(mainTestUserName, secondTestUserName);
        assertEquals(1, result.size());
        verify(userRepository).findByUsername(mainTestUserName);
        verify(userRepository).findFriends(mainTestUser, secondTestUserName);
    }

    @Test
    void friendsShouldCallCorrectRepositoryMethodBasedWithoutQuery(@Mock UserRepository userRepository) {
        UserWithStatus friend =
                new UserWithStatus(
                        secondTestUser.getId(),
                        secondTestUser.getUsername(),
        ...
        when(userRepository.findByUsername(mainTestUserName))
                .thenReturn(Optional.of(mainTestUser));
        when(userRepository.findFriends(mainTestUser))
                .thenReturn(List.of(friend));

        userService = new UserService(userRepository);

        List<UserJsonBulk> result = userService.friends(mainTestUserName, null);
        assertEquals(1, result.size());
        verify(userRepository).findByUsername(mainTestUserName);
    ...
    @Test
    void createFriendshipRequestShouldThrowExceptionForSelfRequest(@Mock UserRepository userRepository) {
        userService = new UserService(userRepository);
    ...
    @Test
    void createFriendshipRequestShouldCreateFriendship(@Mock UserRepository userRepository) {
        when(userRepository.findByUsername(mainTestUserName))
                .thenReturn(Optional.of(mainTestUser));
        ...
        assertTrue(mainTestUser.getFriendshipRequests().stream()
                .anyMatch(f -> f.getAddressee().equals(secondTestUser)
                        && f.getStatus() == FriendshipStatus.PENDING));
    }


    @Test
    void acceptFriendshipRequestShouldThrowExceptionForSelfRequest(@Mock UserRepository userRepository) {
        userService = new UserService(userRepository);
        String username = "testUser";

        SameUsernameException exception = assertThrows(SameUsernameException.class,
                () -> userService.acceptFriendshipRequest(username, username));
    ...
    @Test
    void acceptFriendshipRequestShouldCreateFriendship(@Mock UserRepository userRepository) {
        FriendshipEntity friendship = new FriendshipEntity();
        friendship.setRequester(secondTestUser);
        friendship.setAddressee(mainTestUser);
        friendship.setStatus(FriendshipStatus.PENDING);

        mainTestUser.setFriendshipAddressees(List.of(friendship));
    ...
    @Test
    void acceptFriendshipRequestShouldThrowNotFoundExceptionIfNoInvitation(@Mock UserRepository userRepository) {
        FriendshipEntity friendship = new FriendshipEntity();
        friendship.setRequester(thirdTestUser);
        friendship.setAddressee(mainTestUser);
        friendship.setStatus(FriendshipStatus.PENDING);
        ...
        assertEquals("Can`t find invitation from username: '" + secondTestUserName + "'",
                exception.getMessage(),
